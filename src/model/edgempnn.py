...
def extract_patch_sides(patches: Tensor, trim: int = 0) -> Tensor:
    ...
    return torch.stack((top, right, bottom, left), dim=1)  # [N, 4, C, L]
...
class Border_GAT(nn.Module):
    ...
    def __init__(
        ...
        self.border_mlp = nn.Sequential(
            nn.Linear(self.border_embedding.out_dim, 32),
            nn.GELU(),
            nn.Linear(32,32)
        )
    ...
    def forward(self, xy_pos, time, patch_rgb, patch_borders,  edge_index, batch):
    ...
    def compute_edge_features(self, patches, edge_index):
        cos = nn.CosineSimilarity(dim=0) #Initialize cosine similarity function
        # patches: [num_patches, C, H, W]
        num_edges = edge_index.shape[1]
        edge_feats = []
        
        # Fix to avoid recomputing edge features for the same patch multiple times
        for i, j in edge_index.t():
            borders_i = [BorderEmbedding(b) for b in get_borders(patches[i])] # top_i, bottom_i, left_i, right_i
            borders_j = [BorderEmbedding(b) for b in get_borders(patches[j])] # top_j, bottom_j, left_j, right_j
            
            
            pairwise_borders = list(itertools.product(borders_i, borders_j))
            
            # Compute cosine similarity between border features of the two patches
            border_feats = [cos(border_feat_i, border_feat_j) for border_feat_i, border_feat_j in pairwise_borders]
            
            # Concatenate
            border_feats = torch.stack(border_feats).reshape(-1)
            
            #Pass through MLP to get final edge feature vector
            edge_feat = self.border_mlp(border_feats)
